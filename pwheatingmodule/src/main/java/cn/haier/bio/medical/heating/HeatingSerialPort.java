...
class HeatingSerialPort implements PWSerialPortListener {
    ...
    private WeakReference<IHeatingListener> listener;

    public HeatingSerialPort() {
    }
    ...
    private void createHelper(String path) {
        if (this.helper == null) {
            this.helper = new PWSerialPortHelper("HeatingSerialPort");
    ...
    private void createHandler() {
        if (this.thread == null && this.handler == null) {
            this.thread = new HandlerThread("HeatingSerialPort");
            this.thread.start();
    ...
    private void write(byte[] data) {
        ...
        if (null != this.listener && null != this.listener.get()) {
            this.listener.get().onHeatingPrint("HeatingSerialPort Send:" + HeatingTools.bytes2HexString(data, true, ", "));
        }
    }

    private void switchReadModel() {
        if (null != this.listener && null != this.listener.get()) {
            this.listener.get().onHeatingSwitchReadModel();
        }
    }

    private void switchWriteModel() {
        if (null != this.listener && null != this.listener.get()) {
            this.listener.get().onHeatingSwitchWriteModel();
        }
    }
    ...
    public void onConnected(PWSerialPortHelper helper) {
        ...
        if (null != this.listener && null != this.listener.get()) {
            this.listener.get().onHeatingConnected();
        }
    }
    ...
    public void onReadThreadReleased(PWSerialPortHelper helper) {
        ...
        if (null != this.listener && null != this.listener.get()) {
            this.listener.get().onHeatingPrint("HeatingSerialPort read thread released");
        }
    }
    ...
    public void onException(PWSerialPortHelper helper, Throwable throwable) {
        ...
        this.ready = false;
        if (null != this.listener && null != this.listener.get()) {
            this.listener.get().onHeatingException(throwable);
        }
    }
    ...
    public void onStateChanged(PWSerialPortHelper helper, PWSerialPortState state) {
        ...
        if (null != this.listener && null != this.listener.get()) {
            this.listener.get().onHeatingPrint("HeatingSerialPort state changed: " + state.name());
        }
    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0x10: {
                    ...
                    byte[] response = HeatingTools.packageStateResponse(data);
                    HeatingSerialPort.this.write(response);
                    HeatingSerialPort.this.switchReadModel();
                    if (null != HeatingSerialPort.this.listener && null != HeatingSerialPort.this.listener.get()) {
                        HeatingSerialPort.this.listener.get().onHeatingDataChanged(data);
                    }
                    break;
                }
                case 0x03: {
                    byte[] response = null;
                    if (null != HeatingSerialPort.this.listener && null != HeatingSerialPort.this.listener.get()) {
                        response = HeatingSerialPort.this.listener.get().packageHeatingResponse(msg.arg1);
                    }
